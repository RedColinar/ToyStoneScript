...
public class Parser {
    ...
    private Parser(Class<? extends ASTree> clazz) {
        reset(clazz);
    }
    ...
    public static Parser rule(String name) {
        Parser p = new Parser((Class<? extends ASTree>) null);
        p.name = name;
        return p;
    }
    ...
    private Parser reset(Class<? extends ASTree> clazz) {
        elements = new ArrayList<>();
        factory = Factory.getForASTList(clazz);
        if (clazz != null) {
            name = clazz.getSimpleName() + "Parser";
        }
        return this;
    }
